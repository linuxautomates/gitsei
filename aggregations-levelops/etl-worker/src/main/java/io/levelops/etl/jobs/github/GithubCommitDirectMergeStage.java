...
@Log4j2
@Service
public class GithubCommitDirectMergeStage extends BaseIngestionResultProcessingStage<GithubRepository, GithubJobState> {
    private final GithubAggHelperService helper;

    public GithubCommitDirectMergeStage(GithubAggHelperService helper) {
        this.helper = helper;
    }

    @Override
    public String getName() {
        return "Github Commit Direct Merge Stage";
    }
    ...
    public void process(JobContext context, GithubJobState jobState, String ingestionJobId, GithubRepository entity) throws SQLException {
        helper.updateRepositoryCommitsForDirectMerge(entity, context.getTenantId(), context.getIntegrationId());
    }
