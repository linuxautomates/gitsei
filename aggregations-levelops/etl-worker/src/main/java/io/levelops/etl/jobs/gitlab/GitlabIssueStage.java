...
@Service
public class GitlabIssueStage extends BaseIngestionResultProcessingStage<GitlabIssue, GitlabState>  {
    private final GitlabAggHelperService gitlabAggHelperService;

    @Autowired
    public GitlabIssueStage(GitlabAggHelperService gitlabAggHelperService) {
        this.gitlabAggHelperService = gitlabAggHelperService;
    }

    @Override
    public void process(JobContext context, GitlabState jobState, String ingestionJobId, GitlabIssue entity) throws SQLException {
        gitlabAggHelperService.processIssue(context.getTenantId(), context.getIntegrationId(), entity);
    }
