...
public class BAHistAssigneeTimesCompositeConverter implements BAESResultConverter {
    private final String categoryName;
    private final ESAggInterval aggInterval;

    public static BAHistAssigneeTimesCompositeConverterBuilder builder(String categoryName, ESAggInterval aggInterval) {
    ...
    public List<FTEPartial> convert(SearchResponse<Void> sr) {
        ...
        sr.aggregations().get("nested_w_hist_assignee_statuses").nested().aggregations().get("combined").composite().buckets().array().forEach( ca -> {
            Long effortOrTotalEffort = (long) ca.aggregations().get("filter_w_hist_assignee_statuses.issue_status_category").filter().aggregations().get("sum_w_hist_assignee_statuses.hist_assignee_time").sum().value();
