...
public class BAHistAssigneeCountsConverter implements BAESResultConverter {
    private final String categoryName;
    private final ESAggInterval aggInterval;
    private final Calculation calculation;

    public static BAHistAssigneeCountsConverterBuilder builder(String categoryName, ESAggInterval aggInterval, Calculation calculation) {
    ...
    public List<FTEPartial> convert(SearchResponse<Void> sr) {
        ...
        sr.aggregations().get("nested_w_hist_assignee_statuses").nested().aggregations().get("across_w_hist_assignee_statuses.historical_assignee_id").sterms().buckets().array().forEach(term1 -> {
            term1.aggregations().get("across_w_hist_assignee_statuses.historical_assignee").sterms().buckets().array().forEach( term2 -> {
                term2.aggregations().get("reverse_sum_story_points_by_month").reverseNested().aggregations().get("date_histo_w_resolved_at").dateHistogram().buckets().array().forEach( term3 -> {
