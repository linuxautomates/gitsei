...
@Log4j2
@Service
public class WorkItemsResponseTimeReportService {
    ...
    public WorkItemsResponseTimeReportService(DataSource dataSource,
    ...
    public DbListResponse<DbAggregationResult> generateReport(String company, WorkItemsFilter filter,
        ...
        selectFields.addAll(List.of(
                Query.selectField("*"),
                Query.selectField("(extract(epoch from coalesce(first_comment_at, now()))" +
                        " - extract(epoch from workitem_created_at))", "response_time")
        ));
