...
public class ScmFilterParserCommons {
    public static final Set<String> SCM_APPLICATION_TYPES = Set.of("github", "bitbucket", "gitlab", "helix", "gerrit", "azure_devops");
    private final ProductsDatabaseService productsDatabaseService;

    @Autowired
    public ScmFilterParserCommons(ProductsDatabaseService productsDatabaseService) {
    ...
    public Map<Integer, Map<String, Object>> getProductFilters(String company, Set<UUID> productIds) throws SQLException {
        DbListResponse<DBOrgProduct> productDbListResponse = productsDatabaseService.filter(company,
                QueryFilter.builder()
                        ...
                        .strictMatch("integration_type", SCM_APPLICATION_TYPES)
                        .build(), 0, 10);
