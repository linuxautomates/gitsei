...
/**
 * Factory for obtaining {@link SalesforceClient} for given {@link IntegrationKey}
 */
public class SalesforceClientFactory {
    ...
    private final LoadingCache<IntegrationKey, SalesforceClient> clientCache;

    /**
     * @param inventoryService {@link InventoryService} for getting integration and authentication details
     * @param objectMapper     {@link ObjectMapper} for the {@link SalesforceClient}
     * @param okHttpClient     {@link OkHttpClient} for the  {@link SalesforceClient}
     * @param pageSize         response page size
     */
    @Builder
    public SalesforceClientFactory(InventoryService inventoryService, ObjectMapper objectMapper,
    ...
    /**
     * Loads a {@link SalesforceClient} corresponding to the {@link IntegrationKey}
     * from the {@see LoadingCache<IntegrationKey, SalesforceClient>}
     *
     * @param key {@link IntegrationKey} for the request
     * @return {@link SalesforceClient} corresponding to the {@link IntegrationKey}
     * @throws SalesforceClientException for any exception during creation of a client
     */
    public SalesforceClient get(final IntegrationKey key) throws SalesforceClientException {
        Validate.notNull(key, "key cannot be null");
        try {
            return clientCache.get(key);
        } catch (ExecutionException e) {
            throw new SalesforceClientException(e);
        }
    }

    /**
     * Creates a new {@link SalesforceClient} for the {@link IntegrationKey}. It uses the {@link Integration} and
     ...
     * @return {@link SalesforceClient} created client
     */
    private SalesforceClient getNewClient(final IntegrationKey integrationKey) {
        try {
            ...
            return SalesforceClient.builder()
    ...
    /**
     * Builds an {@link OkHttpClient} for {@code integrationKey} with a {@link OauthTokenInterceptor}
     * with authentication credentials from {@code tokens}
     *
     * @param integrationKey        the {@link IntegrationKey} for which to build the {@link SalesforceClient}
    ...
    private OkHttpClient buildAuthenticatedClient(IntegrationKey integrationKey, List<Token> tokens) throws InventoryException {
        return InventoryHelper.handleTokens(SALESFORCE, integrationKey, tokens,
    ...
    /**
     * builds a {@link SalesforceClient} with authenticated {@link OkHttpClient} for an {@link Integration}
     * with the authentication credentials from {@link Token}. Builds a new {@link SalesforceClient} on each call.
     *
     ...
     * @return {@link SalesforceClient} built for the {@code integration} using {@code token}
     * @throws InventoryException if {@code tokens} is empty or if no supported token is found for the integration
     */
    public SalesforceClient buildFromToken(String tenantId, Integration integration, Token token) throws InventoryException {
