...
function* trellisProfileUpdateSaga(action: any) {
  try {
    const trellisServices = new TrellisProfileServices();

    // @ts-ignore
    const response = yield call(trellisServices.update, action.id, action.data);
    if (response.error) {
...
export function* trellisProfileUpdateWatcherSaga() {
  yield takeLatest(trellisProfileActions.TRELLIS_PROFILE_UPDATE, trellisProfileUpdateSaga);
}
