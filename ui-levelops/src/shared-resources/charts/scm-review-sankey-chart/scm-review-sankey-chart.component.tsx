...
import { ScmReviewSankeyChartProps } from "../chart-types";
...
import {
  ...
  MAX_LENGTH_TO_SHOW,
  RemainingUsersKeys,
  reviewersSortingOptions,
  SCMCollabSortingValues,
  SCMReviewCollaborationCustomStateType,
  submittersSortingOptions
} from "./constant";
import { CommitterReviewerConnectionType, SCMCollabUsersConfigType } from "./scm-review-chart.types";
import SCMReviewCollborationFooterComponent from "./scm-review-collaboration.footer";
import {
  getSCMReviewCollabSlicedUserList,
  reviewCollaborationPartialListSorting,
  SCMReviewListToPercentageList
} from "./scm-review-collaboration.helper";
import "./scm-review-sankey-chart.scss";
import SCMReviewTotalBreakdownComponent from "./scm-review-total-breakdown.component";
import SCMReviewUserListHeaderActionComponent from "./scm-user-list-header.component";
import useId from "custom-hooks/useId";
import { WidgetFilterContext } from "dashboard/pages/context";

const ScmReviewSankeyChartComponent: React.FC<ScmReviewSankeyChartProps> = (props: ScmReviewSankeyChartProps) => {
  const { data: transformed, onClick } = props;
  ...
  const modifiedCollabMapping = useMemo(() => {
    const approvalStatus = get(filters, [props.id, "collab_states"], []).map((item: any) => item?.replaceAll("-", "_"));
  ...
  const maxSubmitterValue = useMemo(() => maxBy(submittersList, "total_prs")?.total_prs || 0, []);
  const maxReviewerValue = useMemo(() => maxBy(reviewersList, "total_prs")?.total_prs || 0, []);
  ...
  const finalReviewerList = useMemo(() => {
    ...
    if (showBreakdown?.[key] === true && key?.includes("submitter")) {
      ...
      const allRecordsOfCurSubm = (apiData ?? []).filter(apiRec => apiRec?.key === startData?.key);
  ...
  const handleOnClick = useCallback((key: "submitter" | "reviewer", value: string, label?: string) => {
    if (key === "reviewer") {
      onClick?.({
        approvers: [value],
        name: label ?? value,
        ou_exclusions: ["approvers"]
      });
      return;
    }
    onClick?.({ creators: [value], name: label ?? value, ou_exclusions: ["creators"] });
  }, []);
  ...
  const getCommittersHeaderPrefix = useMemo(() => {
    ...
    if (showBreakdown?.[key] === true && key?.includes("reviewer")) {
      const startIndex = parseInt(key.split("_")[0]);
      return `Creators for ${slicedReviewersList[startIndex]?.name ?? ""}`;
    }
    return "Creators";
  ...
  return (
    <div>
      <SCMReviewTotalBreakdownComponent totalBreakdown={totalBreakdowndata} />
                ...
                <div className="submitters-header">
                  ...
                  <span style={{ fontSize: 16, width: "50%" }}>
                    ...
                    <AntSelect
                      ...
                      options={submittersSortingOptions}
                ...
                <div className="reviewers-header">
                  ...
                  <span style={{ fontSize: 16, width: "50%" }}>
                    ...
                    <AntSelect
                      ...
                      options={reviewersSortingOptions}
                  ...
                  {map(slicedReviewersList, (item, index) => {
                    return (
                      <div
                        ...
                        onClick={() => {
                          if (![RemainingUsersKeys.OTHER_REVIEWER].includes(item?.key as any)) {
...
export default ScmReviewSankeyChartComponent;
