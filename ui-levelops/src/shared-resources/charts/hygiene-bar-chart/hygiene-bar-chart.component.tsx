...
import { getInitialFilters, yAxisIdByKey } from "../helper";
...
const HygieneBarChartComponent = (props: BarChartProps) => {
  const {
    data,
    barProps,
    chartProps,
  ...
  const onBarClick = useCallback((data: any, hygiene?: string) => {
    ...
    if (data && hasClickEvents && onClick) {
      if (onChartClickPayload) {
        const args = { data, across: props.xUnit, visualization: ChartType.HYGIENE_BAR_CHART, hygiene };
      ...
      return (barProps || []).map((bar, i: number) => {
        ...
        return (
          <Bar
            ...
            key={`bar-${i}`}
            onClick={(args: any) => onBarClick(args, bar?.dataKey)}
            {...bar}
          />
        );
      });
  ...
  const renderLegends = useMemo(() => {
    return (
      <ChartLegendComponent
        ...
        labelMapping={hygieneMapping}
      />
    );
  }, [filters, allowLabelTransform, hygieneMapping]);
...
export default HygieneBarChartComponent;
