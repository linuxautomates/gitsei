...
import { ignoreKeys } from "./bar-chart.ignore-keys";
...
import { generateStackFilters } from "./bar-chart.helper";
import { labelMapping } from "../constant";
import { BarChartTooltipWrapper } from "./BarChartTooltipWrapper";
...
const BarChartComponent = (props: BarChartProps) => {
  ...
  const updatedData = useMemo(() => {
    if (props.useOrderedStacks && props.orderedBarProps) {
      return data.map((item: any) => {
        ...
        return {
          name: item.name,
          totalStacksBarCount: item?.totalStacksBarCount,
          ...(keys || {})
        };
      });
    }
  ...
  const onBarClick = useCallback(
    (data: any) => {
      ...
      if (data && hasClickEvents && onClick) {
        const selectedFilteredList = generateStackFilters(apiData, filters, id);
  ...
  const tooltipContent = useCallback(
    ...
    [props, isHoverOnTooltip, stacked]
  );
  ...
  const bars = useMemo(() => {
    if (stacked) {
      if (props.useOrderedStacks && props.orderedBarProps) {
        return (props.orderedBarProps || []).map((bar, i) => {
          let yAxisId = "hidden";
  ...
  return (
    <ResponsiveContainer>
      <BarChart
        ...
        {baseLinesDataPoints && baseLinesDataPoints?.length && (
          <YAxis
            orientation="right"
        ...
        {bars}
        {baseLinesDataPoints && referenceAreas}
      </BarChart>
    </ResponsiveContainer>
  );
};
