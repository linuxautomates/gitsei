...
import { leadTimeandMTTRTransformerDrilldownRecords } from "dashboard/helpers/drilldown-transformers/leadTimeDrilldownTransformer";
...
export const getFilterConfig = (params: any) => {
  const IMApplication = get(params.workspaceProfile, ["lead_time_for_changes", "issue_management_integrations", 0]);
...
export const getDoraProfileIntegrationType = (params: any) => {
  ...
  const IMApplication = get(workspaceOuProfilestate, ["lead_time_for_changes", "issue_management_integrations", 0]);
  switch (event) {
    case "commit_created":
      return "SCM";
    case "api_event":
...
export const getDoraExtraDrilldownProps = (params: any) => {
  const { widgetId, drillDownProps } = params;
  const activeColumn = get(drillDownProps, ["x_axis", "histogram_stage_name"], "total");
  return {
    hasDynamicColumns: true,
    renderDynamicColumns: renderVelocityStageDynamicColumns as any,
    scroll: { x: "fit-content" },
    widgetId: widgetId as string,
    shouldDerive: ["integration_id"],
    derive: true,
    transformRecordsData: leadTimeandMTTRTransformerDrilldownRecords,
    activeColumn
  };
};
...
export const getDoraLeadTimeData = (params: any) => {
  const { workspaceOuProfilestate } = params;
  return workspaceOuProfilestate.lead_time_for_changes;
};
