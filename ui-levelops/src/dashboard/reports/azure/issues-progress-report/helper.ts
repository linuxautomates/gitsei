...
import { azureIssuesReportMetricsChartMapping, TOTAL_NUMBER_OF_TICKETS, TOTAL_SUM_OF_STORY_POINTS } from "./constant";
import { AzureIssuesReportMetricTypes } from "./enums";

export const getTotalLabel = (data: { unit: string }) => {
  const { unit } = data;
  return unit === "Story Points" ? TOTAL_SUM_OF_STORY_POINTS : TOTAL_NUMBER_OF_TICKETS;
};
