...
import IntegrationFields from "../IntegrationFieldsNew";
...
import { isAzure, isJira, isZendesk } from "helper/integration.helper";
...
const IntegrationDetailsNewContainer: React.FC<IntegrationDetailsNewContainerProps> = props => {
  ...
  }, [customFieldMappingLoading, mappedCustomHygienes, integration_form]);
  ...
  return (
    <div className="integration-details-container">
      ...
      {isAzure(application) && <div className="integration-row">{renderAddCustomHygienes}</div>}
      {isJira(application) && <div className="integration-row">{renderAddCustomHygienes}</div>}
    </div>
  );
};
