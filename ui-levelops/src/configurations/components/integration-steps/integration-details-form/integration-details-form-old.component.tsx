...
import { integrationStringUtility } from "configurations/constants";
...
const IntegrationDetailsForm: React.FC<IntegrationDetailsFormProps> = props => {
  const { integration_form, tags_loading, updateField } = props;
  ...
  return (
    <Form layout="vertical">
      <div className="integration-info-grid">
        ...
        />
        {[IntegrationTypes.JIRA].includes(integration_form.application) && (
        ...
        {[integrationStringUtility.AZURE_DEVOPS].includes(integration_form.application) && (
