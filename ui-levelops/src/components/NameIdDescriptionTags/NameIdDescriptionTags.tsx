...
export const NameId = (props: NameIdProps): JSX.Element => {
  const { identifierProps, nameLabel = "Name", inputGroupProps = {} } = props;
  const newInputGroupProps = { placeholder: "Enter Name", ...inputGroupProps };
  return (
  ...
  return (
    <Container style={{ marginBottom: isDescriptionOpen ? "0" : "var(--spacing-medium)" }}>
      <Label className={cx(Classes.LABEL, css.descriptionLabel)} data-tooltip-id={props.dataTooltipId}>
        {isOptional ? "Description (optional)" : "Description"}
...
export function NameIdDescriptionTags(props: NameIdDescriptionTagsProps): JSX.Element {
  ...
  const newInputGroupProps = { placeholder, ...inputGroupProps };
  return (
    <Container className={cx(css.main, className)}>
      {isIdentifierHidden ? (
        <FormInput.Text name="name" placeholder={placeholder} label={identifierProps?.inputLabel} />
      ) : (
